...
ruta_archivo_imu = '/home/darwin/Documents/DARwInOP2_ws/src/RetoTRYTA/scripts/datos_imu.csv'


# Crear el archivo CSV si no existe y agregar el encabezado
def inicializar_csv():
    try:
        pd.read_csv(ruta_archivo_imu)
    except FileNotFoundError:
        df = pd.DataFrame(columns=["orientacion_x", "orientacion_y", "orientacion_z"])
        df.to_csv(ruta_archivo_imu, index=False)
...
def procesar_datos_imu(datos, guardar_en_csv=False):
    ...
    if guardar_en_csv:
        ...
        df.to_csv(ruta_archivo_imu, mode='a', header=False, index=False)
...
def subir_escalon():
    ...
    idx_tobillo_derecho = estados_articulacion.name.index("r_ank_roll")
    idx_tobillo_izquierdo = estados_articulacion.name.index("l_ank_roll")

    for _, paso in ciclo_escaleras.iloc[:, 1:-1].items():
        ...
        if datos_imu_actual:
            if datos_imu_actual["orientacion_y"] < 0.00:
                ...
                fila_radianes[idx_tobillo_izquierdo] -= correccion
            elif datos_imu_actual["orientacion_y"] > 0.01:
                correccion = datos_imu_actual["orientacion_x"] * (1 / 100)
                fila_radianes[idx_tobillo_derecho] += correccion
...
ciclo_escaleras = pd.read_csv('/home/darwin/Documents/DARwInOP2_ws/src/SubirEscaleras/scripts/Secuencia_Ascenso.csv')
...
# Descomentar las siguientes línea si se desea guardar datos
# inicializar_csv()
